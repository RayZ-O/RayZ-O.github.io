...
struct AVLTreeNode {
    ...
    AVLTreeNode *left;
    AVLTreeNode *right;
    AVLTreeNode(T k, U v) : key(k), value(v), height(0),
...
{% highlight c++ %}
template <typename T, typename U>
AVLTreeNode<T, U>* AVLTree<T, U>::AVLInsert(T key, U value) {
...
{% endhighlight %}
The *InsertBalance* function loop from the parent of newly inserted node to the root, re-balance a node if its balance factor is greater than 1 or less than -1. Stop if meets a node's balance factor equal to 0. At most 1 rotation is perform in InsertBalance.
